 ...
 * messages are narrowed here, to `app.s2`, so the payload does not carry the other nine slices'
...
export async function TasksIntl({ children }: { children: ReactNode }) {
  ...
  return (
    <NextIntlClientProvider messages={{ app: { s2: app.s2 as AbstractIntlMessages } }}>
      {children}
