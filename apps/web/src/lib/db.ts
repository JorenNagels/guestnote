 ...
 *
 * No raw handle escapes this module beyond `Db`, and `Db` has no unscoped read methods
 * that matter -- every query goes through `withTenant` or `withUser`, both of which open
 * a transaction with the tenant GUCs already set. The escape hatch is
 * `@guestnote/db/unsafe`, which Biome and a git-grep test both forbid outside
 * `packages/db/**`.
 */
