...
describe('getWeddingDetail', () => {
  it('reads the notes for an owner and for an assigned member', async () => {
    expect((await getWeddingDetail(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1)))?.slug).toBe(
      'a-one',
    )
    expect((await getWeddingDetail(WeddingScope.of(h.db, member, F.orgA, F.weddingA1)))?.slug).toBe(
      'a-one',
    )
  })
  ...
  it('is null for a member on a wedding they are not assigned to, and for another org', async () => {
    expect(await getWeddingDetail(WeddingScope.of(h.db, member, F.orgA, F.weddingA2))).toBeNull()
    expect(
      await getWeddingDetail(WeddingScope.of(h.db, otherOrgOwner, F.orgA, F.weddingA1)),
    ).toBeNull()
    expect(await getWeddingDetail(WeddingScope.of(h.db, owner, F.orgA, F.weddingB1))).toBeNull()
  })
})
...
describe('updateWedding', () => {
  ...
  it('saves every editable field for an owner and an assigned member', async () => {
    const saved = unwrap(
      await updateWedding(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), EDIT),
    )
    ...
    const again = unwrap(
      await updateWedding(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), {
        ...EDIT,
        color: null,
        headcount: null,
      }),
  ...
  it('refuses a couple, and writes nothing', async () => {
    expect(await updateWedding(WeddingScope.of(h.db, couple, F.orgA, F.weddingA1), EDIT)).toEqual(
      NOT_FOUND,
    )
    expect(
      (await getWeddingDetail(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1)))
        ?.coupleDisplayName,
    ).toBe('A One')
  })

  it("refuses a member on someone else's wedding and a stranger from another org", async () => {
    expect(await updateWedding(WeddingScope.of(h.db, member, F.orgA, F.weddingA2), EDIT)).toEqual(
      NOT_FOUND,
    )
    expect(
      await updateWedding(WeddingScope.of(h.db, otherOrgOwner, F.orgA, F.weddingA1), EDIT),
    ).toEqual(NOT_FOUND)
    expect(await updateWedding(WeddingScope.of(h.db, owner, F.orgA, F.weddingB1), EDIT)).toEqual(
      NOT_FOUND,
    )
    expect(
      (await getWeddingDetail(WeddingScope.of(h.db, otherOrgOwner, F.orgB, F.weddingB1)))
        ?.coupleDisplayName,
    ).toBe('B One')
  })
  ...
  it('rejects a negative headcount at the database', async () => {
    await expect(
      updateWedding(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), { ...EDIT, headcount: -1 }),
    ).rejects.toThrow()
  })
})
...
describe('getWeddingTaskCounts', () => {
  it('counts what the fixture holds: two open, none done, none late', async () => {
    expect(await getWeddingTaskCounts(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1))).toEqual({
      total: 2,
      open: 2,
      done: 0,
      overdue: 0,
  ...
  it("is all zeros for a user with no standing, never another wedding's numbers", async () => {
    ...
    expect(await getWeddingTaskCounts(WeddingScope.of(h.db, couple, F.orgA, F.weddingA1))).toEqual(
      zero,
    )
    expect(
      await getWeddingTaskCounts(WeddingScope.of(h.db, otherOrgOwner, F.orgA, F.weddingA1)),
    ).toEqual(zero)
    // A1's tasks must not show up under A2's id for an owner either.
    expect(
      (await getWeddingTaskCounts(WeddingScope.of(h.db, owner, F.orgA, F.weddingA2))).total,
    ).toBe(1)
  })
})
...
describe('events', () => {
  ...
  it("lists a wedding's events, trimmed to HH:MM and never another wedding's", async () => {
    const rows = await listWeddingEvents(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1))
    expect(rows).toEqual([
      {
        id: F.eventA1,
        label: 'Ceremony',
  ...
  it('orders by date, then time, with no time last', async () => {
    await createWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), {
      ...EVENT,
      label: 'No time',
      startsAt: null,
      startsOn: '2027-07-31',
    ...
    const labels = (await listWeddingEvents(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1))).map(
      (e) => e.label,
    )
  ...
  it("appends at the end of the wedding's positions", async () => {
    const a = unwrap(
      await createWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), EVENT),
    )
    const b = unwrap(
      await createWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), EVENT),
    )
  ...
  it('lets an assigned member create, edit and remove on their wedding', async () => {
    const made = unwrap(
      await createWeddingEvent(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), EVENT),
    )
    expect(made).not.toBeNull()
    const id = made?.id ?? ''
    expect(
      unwrap(
        await updateWeddingEvent(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), id, {
          ...EVENT,
          label: 'Renamed',
        }),
      )?.label,
    ...
    expect(
      await deleteWeddingEvent(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), id),
    ).toEqual({
      ok: true,
      value: null,
    })
    expect(
      (await listWeddingEvents(WeddingScope.of(h.db, member, F.orgA, F.weddingA1))).map(
        (e) => e.id,
      ),
    ).toEqual([F.eventA1])
  })
  ...
  it('removes softly: the row stays, so a run sheet item keeps its event', async () => {
    await deleteWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), F.eventA1)
    expect(await listWeddingEvents(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1))).toEqual([])
  ...
  it('refuses a couple and a member on an unassigned wedding, for every verb', async () => {
    for (const [who, wedding] of [
      ...
      expect(await createWeddingEvent(WeddingScope.of(h.db, who, F.orgA, wedding), EVENT)).toEqual(
        NOT_FOUND,
      )
      expect(await listWeddingEvents(WeddingScope.of(h.db, who, F.orgA, wedding))).toEqual([])
      expect(
        await updateWeddingEvent(WeddingScope.of(h.db, who, F.orgA, wedding), F.eventA1, EVENT),
      ).toEqual(NOT_FOUND)
      expect(
        await deleteWeddingEvent(WeddingScope.of(h.db, who, F.orgA, wedding), F.eventA1),
      ).toEqual(NOT_FOUND)
    }
    expect(await listWeddingEvents(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1))).toHaveLength(
      1,
    )
  })
  ...
  it('does not let an owner reach across weddings or orgs through the wrong id', async () => {
    expect(
      await updateWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), F.eventA2, EVENT),
    ).toEqual(NOT_FOUND)
    expect(
      await deleteWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), F.eventA2),
    ).toEqual(NOT_FOUND)
    expect(
      await createWeddingEvent(WeddingScope.of(h.db, owner, F.orgA, F.weddingB1), EVENT),
    ).toEqual(NOT_FOUND)
    expect(await listWeddingEvents(WeddingScope.of(h.db, owner, F.orgA, F.weddingA2))).toHaveLength(
      1,
    )
    expect(
      await listWeddingEvents(WeddingScope.of(h.db, otherOrgOwner, F.orgB, F.weddingB1)),
    ).toHaveLength(1)
  })
  ...
  it('rejects an event on a wedding that does not exist', async () => {
    expect(
      await createWeddingEvent(
        WeddingScope.of(h.db, owner, F.orgA, '00000000-0000-0000-0000-000000000000'),
        EVENT,
      ),
    ).toEqual(NOT_FOUND)
  })
