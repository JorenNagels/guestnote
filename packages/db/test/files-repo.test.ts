...
describe('listFiles', () => {
  it('lists one wedding and one kind for an owner, not the whole org', async () => {
    const a1 = (await listFiles(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), 'file'))?.map(
      (f) => f.id,
    )
    ...
    expect(
      (await listFiles(WeddingScope.of(h.db, owner, F.orgA, F.weddingA2), 'image'))?.map(
        (f) => f.id,
      ),
  ...
  it('reads the uploader as a name or an address, never blank', async () => {
    const row = pending()
    await createPendingFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row)
    await confirmFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)
    const found = await getFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)
  ...
  it('shows an assigned member their wedding and refuses its sibling', async () => {
    const own = (await listFiles(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), 'file'))?.map(
      (f) => f.id,
    )
    expect(own).toEqual(expect.arrayContaining([F.fileA1Internal, F.fileA1Shared]))
    expect(await listFiles(WeddingScope.of(h.db, member, F.orgA, F.weddingA2), 'image')).toBeNull()
  })

  it('answers null, not empty, for anyone without standing', async () => {
    expect(await listFiles(WeddingScope.of(h.db, ownerB, F.orgA, F.weddingA1), 'file')).toBeNull()
    // Own org, another org's wedding: RLS hides every row, so there is nothing to list.
    expect(await listFiles(WeddingScope.of(h.db, ownerB, F.orgB, F.weddingA1), 'file')).toEqual([])
    // A couple is refused before a transaction opens (spec 0003).
    expect(await listFiles(WeddingScope.of(h.db, couple, F.orgA, F.weddingA1), 'file')).toBeNull()
  })
})
...
describe('the pending lifecycle', () => {
  it('hides a row until it is confirmed, then shows it', async () => {
    ...
    expect(await createPendingFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row)).toEqual(
      {
        ok: true,
        value: null,
      },
    ...
    expect(await getFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)).toBeNull()
    expect(
      (await listFiles(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), 'image'))?.some(
        (f) => f.id === row.id,
      ),
    ).toBe(false)
  ...
  it('refuses a confirm from anyone but the uploader', async () => {
    const row = pending()
    await createPendingFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row)
    expect(await confirmFile(WeddingScope.of(h.db, admin, F.orgA, F.weddingA1), row.id)).toEqual(
      NOT_FOUND,
    )
    expect(await getFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)).toBeNull()
  })
  ...
  it('does not let a confirm bring a deleted file back', async () => {
    ...
    await createPendingFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row)
    await confirmFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)
    expect(await removeFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)).toEqual({
      ok: true,
      value: null,
    })

    expect(await confirmFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)).toEqual(
      NOT_FOUND,
    )
    expect(await getFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)).toBeNull()
  })
  ...
  it('will not point a row at a wedding the caller cannot see', async () => {
    ...
    expect(await createPendingFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingB1), row)).toEqual(
      NOT_FOUND,
    )
  })
  ...
  it('refuses a couple and an unassigned member the insert', async () => {
    expect(
      await createPendingFile(WeddingScope.of(h.db, couple, F.orgA, F.weddingA1), pending()),
    ).toEqual(NOT_FOUND)
    expect(
      await createPendingFile(WeddingScope.of(h.db, member, F.orgA, F.weddingA2), pending()),
    ).toEqual(NOT_FOUND)
  })
  ...
  it('lets an assigned member upload to their own wedding', async () => {
    const row = pending()
    expect(
      await createPendingFile(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), row),
    ).toEqual({
      ok: true,
      value: null,
    })
    expect(
      await confirmFile(WeddingScope.of(h.db, member, F.orgA, F.weddingA1), row.id),
...
describe('renaming, visibility and removal', () => {
  it('renames and flips visibility of a confirmed file', async () => {
    ...
    expect(
      await renameFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id, 'Floor plan'),
    ).toEqual({
      ok: true,
      value: null,
    })
    ...
    expect(await getFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)).toMatchObject({
      name: 'Floor plan',
      visibility: 'internal',
    })
  })
  ...
  it('cannot rename a pending row, nor a row through the wrong wedding', async () => {
    ...
    await createPendingFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row)
    expect(
      await renameFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id, 'x'),
    ).toEqual(NOT_FOUND)

    await confirmFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), row.id)
    expect(
      await renameFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA2), row.id, 'x'),
    ).toEqual(NOT_FOUND)
    expect(await removeFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA2), row.id)).toEqual(
      NOT_FOUND,
    )
  })
  ...
  it('refuses every write to a couple and to another org', async () => {
    expect(
      await removeFile(WeddingScope.of(h.db, couple, F.orgA, F.weddingA1), F.fileA1Shared),
    ).toEqual(NOT_FOUND)
    expect(
      await removeFile(WeddingScope.of(h.db, ownerB, F.orgA, F.weddingA1), F.fileA1Shared),
    ).toEqual(NOT_FOUND)
    expect(
      await getFile(WeddingScope.of(h.db, owner, F.orgA, F.weddingA1), F.fileA1Shared),
    ).not.toBeNull()
  })
})
